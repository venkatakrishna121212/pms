...
class Registerform(forms.ModelForm):
    class Meta:
        model = StudentDB
        fields = ['s_name', 'dob', 'emailid', 'contactno']
...
def get_offer(request):
    if request.session.has_key('username'):
        ...
        form = AppliedJob.objects.filter(
            stdid=sid)  # retrieve offer letters from applied jobs with jobid and attachment download link
